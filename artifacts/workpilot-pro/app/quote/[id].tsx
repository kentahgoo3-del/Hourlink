...
<body><div class="page">
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (!isAvailable) { setSendingEmail(false); return; }
      const logoDataUri = await getLogoDataUri(logoUri);
      const logoHtml = logoDataUri
        ? `<img src="${logoDataUri}" style="height:56px;max-width:180px;object-fit:contain;display:block;margin-bottom:0;" alt="logo"/>`
        : `<div style="font-size:26px;font-weight:900;color:#1e293b;">${companyName}</div>`;
      ...
      const itemRows = quote.items.map((item) => `
        <tr>
          <td style="padding:12px 16px;border-bottom:1px solid #f1f5f9"><div style="font-size:13px;font-weight:600;color:#1e293b">${item.description}</div><div style="font-size:11px;color:#94a3b8;margin-top:2px">${settings.currency}${item.unitPrice.toLocaleString("en-ZA", { minimumFractionDigits: 2 })}/unit</div></td>
          <td style="padding:12px 16px;border-bottom:1px solid #f1f5f9;text-align:center;font-size:13px;color:#64748b">${item.quantity}</td>
          <td style="padding:12px 16px;border-bottom:1px solid #f1f5f9;text-align:right;font-size:13px;font-weight:700;color:#1e293b">${fmtP(item.quantity * item.unitPrice)}</td>
        </tr>`).join("");
      const addrLines = [
...
<body><div class="page">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;padding-bottom:24px;border-bottom:2px solid #e2e8f0">
    <div>${logoHtml}${logoDataUri ? `<div style="font-size:18px;font-weight:800;color:#1e293b;margin-top:10px">${companyName}</div>` : ""}${companyProfile.tagline ? `<div style="font-size:12px;color:#94a3b8;margin-top:3px">${companyProfile.tagline}</div>` : ""}${addrLines ? `<div style="font-size:11px;color:#94a3b8;margin-top:10px;line-height:1.7">${addrLines}</div>` : ""}</div>
    <div style="text-align:right"><div style="font-size:32px;font-weight:900;color:${ACCENT};letter-spacing:-1px">QUOTATION</div><div style="font-size:15px;font-weight:700;color:#1e293b;margin-top:4px">${quote.quoteNumber}</div><div style="display:inline-block;margin-top:8px;background:${ACCENT}18;color:${ACCENT};border:1px solid ${ACCENT}60;border-radius:20px;padding:3px 12px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px">${quote.status}</div></div>
  </div>
  <div style="display:flex;gap:40px;margin-bottom:28px"><div><div style="font-size:9px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#94a3b8;margin-bottom:5px">DATE</div><div style="font-size:13px;font-weight:600">${formatDate(quote.createdAt)}</div></div><div><div style="font-size:9px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#94a3b8;margin-bottom:5px">VALID UNTIL</div><div style="font-size:13px;font-weight:600">${formatDate(quote.validUntil)}</div></div></div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;padding:20px 0;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;margin-bottom:28px"><div><div style="font-size:9px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#94a3b8;margin-bottom:8px">FROM</div><div style="font-size:14px;font-weight:700;margin-bottom:4px">${companyName}</div>${addrLines ? `<div style="font-size:12px;color:#64748b;line-height:1.7">${addrLines}</div>` : ""}</div><div><div style="font-size:9px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#94a3b8;margin-bottom:8px">QUOTED FOR</div><div style="font-size:14px;font-weight:700;margin-bottom:4px">${client?.name || "Client"}</div>${client?.company ? `<div style="font-size:12px;color:#64748b">${client.company}</div>` : ""}${client?.email ? `<div style="font-size:12px;color:#64748b">${client.email}</div>` : ""}${client?.phone ? `<div style="font-size:12px;color:#64748b">${client.phone}</div>` : ""}</div></div>
  ${quote.title ? `<div style="font-size:15px;font-weight:700;margin-bottom:16px">${quote.title}</div>` : ""}
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:28px"><div style="width:280px"><div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f1f5f9"><span style="font-size:13px;color:#64748b">Subtotal</span><span style="font-size:13px">${fmtP(sub)}</span></div>${quote.taxPercent > 0 ? `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f1f5f9"><span style="font-size:13px;color:#64748b">Tax (${quote.taxPercent}%)</span><span style="font-size:13px">${fmtP(taxAmt)}</span></div>` : ""}<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;margin-top:4px;border-top:2px solid #1e293b"><span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px">TOTAL</span><span style="font-size:24px;font-weight:900;color:${ACCENT}">${fmtP(tot)}</span></div></div></div>
  ${quote.notes ? `<div style="background:#f8fafc;border-radius:8px;padding:16px 20px;margin-bottom:16px"><div style="font-size:9px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#94a3b8;margin-bottom:8px">NOTES</div><div style="font-size:13px;color:#64748b;line-height:1.7">${quote.notes}</div></div>` : ""}
  <div style="margin-top:32px;padding-top:16px;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:10px;color:#94a3b8"><span>${companyName}</span><span>Generated by HourLink</span></div>
</div></body></html>`;
      ...
      if (result.status === MailComposer.MailComposerStatus.SENT) {
        updateQuote(id, { status: quote.status === "draft" ? "sent" : quote.status });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <BottomSheet visible={showMenu} onClose={() => setShowMenu(false)} title="Quote Options">
        {client?.email ? (
          <TouchableOpacity
            style={[styles.menuItem, { borderColor: "#ede9fe", backgroundColor: "#f5f3ff" }]}
            onPress={handleSendEmail}
            disabled={sendingEmail}
            testID="send-quote-email-btn"
          >
            {sendingEmail
              ? <ActivityIndicator size="small" color={ACCENT} />
              : <AppIcon name="mail" size={18} color={ACCENT} />}
            <Text style={[styles.menuItemText, { color: ACCENT }]}>
