...
export default function TeamScreen() {
  ...
  const [delegatePriority, setDelegatePriority] = useState<
    "low" | "medium" | "high"
  >("medium");
  ...
  const handleJoin = async () => {
    if (!joinCode.trim()) {
      Alert.alert("Enter the 6-character workspace code.");
      return;
    }
    const name = settings.name || "Member";
    setLoading(true);
    try {
      const ws = await teamSync.joinWorkspace(
  ...
  const handleAddTeamMember = async () => {
    ...
    try {
      ...
      const result = await teamSync.setTeamMembers(
        workspaceCode,
        updated.map((m) => ({ name: m.name, email: m.email, role: m.role })),
      );
      if (result.ok) {
  ...
  const handleDelegate = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Sent!",
        delegateToEmail
          ? `Task assigned to ${delegateToEmail}`
  ...
  const getMemberTimeTotal = (email: string): number => {
    return timeEntries
      .filter(
        (e) =>
          e.memberEmail.toLowerCase() === email.toLowerCase() && e.duration,
      )
  ...
  const getMemberTaskCount = (email: string): number => {
    return teamTasks.filter(
      (t) => t.assignedTo.toLowerCase() === email.toLowerCase(),
    ).length;
  };
  ...
  const portalUrl = workspaceCode
    ? `https://${WEB_APP_DOMAIN}/client-portal/?code=${workspaceCode}&mode=team`
    : "";
  ...
  return (
    ...
    >
        ...
        {!workspace ? (
          <>
            ...
            >
              ...
              <Text
                style={[styles.heroDesc, { color: colors.mutedForeground }]}
              >
                Create a workspace, add team members with auto-generated login
                credentials, and delegate tasks. Members use the Team Portal to
                track time, update progress, and add notes.
              </Text>
            </View>
            <TouchableOpacity
          ...
          <>
            <View style={[styles.wsCard, { backgroundColor: colors.primary }]}>
              ...
              <Text style={styles.wsSub}>
                ...
                {teamMembers.length !== 1 ? "s" : ""} · {teamTasks.length}{" "}
                delegated task{teamTasks.length !== 1 ? "s" : ""}
              </Text>
            </View>

            {isOwner && (
              <View style={styles.tabRow}>
                {(["overview", "members", "tasks", "time"] as const).map(
                  (tab) => (
                    <TouchableOpacity
                      key={tab}
                      style={[
              ...
              <>
                ...
                {isOwner && teamMembers.length > 0 && (
                  ...
                  >
                    ...
                    <Text
                      style={[styles.credBtnText, { color: colors.foreground }]}
                    >
                      {showCredentials
                ...
                {showCredentials &&
                  teamMembers.map((m, i) => (
                    ...
                    >
                      ...
                      >
                        <AppIcon
                          name={copiedEmail === m.email ? "checkmark" : "copy"}
                          size={14}
                          color="#fff"
                        />
                        <Text style={styles.copyBtnText}>
                          {copiedEmail === m.email ? "Copied!" : "Copy Details"}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  ))}
                        ...
                        <View style={styles.pendingTop}>
                          <View
                            style={[
                              ...
                              {
                                backgroundColor:
                                  PRIORITY_COLOR[task.priority] + "22",
                              },
                            ]}
                          >
                            <View
                          ...
                          >
                            from {task.fromUser} · {timeAgo(task.sentAt)}
                          </Text>
                        </View>
                        <Text
                          style={[styles.pTitle, { color: colors.foreground }]}
                        >
                          {task.title}
                        </Text>
                        {task.description ? (
                    ...
                    >
                      ...
                      <View style={styles.memberStats}>
                        ...
                        <View style={styles.statItem}>
                          ...
                          >
                            {formatDuration(getMemberTimeTotal(m.email))}
                          </Text>
                          <Text
                            style={[
                              styles.statLabel,
                      ...
                      <View style={styles.memberActions}>
                        ...
                        >
                          <AppIcon
                            name={
                              copiedEmail === m.email ? "checkmark" : "share"
                            }
                            size={14}
                            color={colors.primary}
                          />
                        ...
                        >
                          ...
                          <Text
                            style={[styles.smallBtnText, { color: "#ef4444" }]}
                          >
                            Remove
                          </Text>
                        </TouchableOpacity>
              ...
              <>
                <View style={styles.taskActions}>
                  ...
                  >
                    ...
                    />
                    <Text
                      style={[styles.bigBtnText, { color: colors.foreground }]}
                    >
                      From My Tasks
                    </Text>
                  </TouchableOpacity>
                ...
                {teamTasks.length === 0 ? (
                  ...
                  teamTasks.map((task) => {
                    const member = teamMembers.find(
                      (m) =>
                        m.email.toLowerCase() === task.assignedTo.toLowerCase(),
                    );
                    const taskTime = timeEntries
                      .filter((e) => e.taskId === task.id && e.duration)
      ...
      >
        ...
        >
          {loading ? (
            ...
            <Text style={styles.bigBtnText}>Join Workspace</Text>
          )}
        </TouchableOpacity>
      </BottomSheet>
      ...
      >
        <Text style={[styles.sheetDesc, { color: colors.mutedForeground }]}>
          Add a member and share their auto-generated login. They'll use the
          Team Portal to view tasks, track time, and update progress.
        </Text>
        <FormField
          label="Name *"
        ...
        <FormField
          ...
          autoCapitalize="none"
        />
        <Text style={[styles.prioLabel, { color: colors.mutedForeground }]}>
          Role
        </Text>
          ...
          {["member", "lead", "contractor"].map((r) => (
            <TouchableOpacity
              ...
              style={[
                ...
                {
                  backgroundColor:
                    memberRole === r ? colors.primary : colors.muted,
                },
              ]}
            ...
            >
              <Text
                style={[
                  styles.prioChipText,
                  { color: memberRole === r ? "#fff" : colors.foreground },
                ]}
              >
            ...
            {appTasks
              ...
              .map((t) => (
                <TouchableOpacity
                  ...
                  onPress={() => selectAppTask(t)}
                >
                  <Text
                    style={[styles.appTaskTitle, { color: colors.foreground }]}
                  >
                    {t.title}
                  </Text>
                  <View
                    style={[
                      styles.pPriority,
                      { backgroundColor: PRIORITY_COLOR[t.priority] + "22" },
                    ]}
                  >
                    <Text
                      style={[
        ...
        {(!delegateFromApp || selectedAppTask) && (
          <>
            <FormField
              ...
              value={delegateTitle}
              onChangeText={setDelegateTitle}
              autoFocus={!delegateFromApp}
            />
            <FormField
              label="Details"
              placeholder="Any extra context..."
              value={delegateDesc}
              onChangeText={setDelegateDesc}
              multiline
              numberOfLines={2}
            />
            <Text style={[styles.prioLabel, { color: colors.mutedForeground }]}>
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        color:
                          delegatePriority === p ? "#fff" : colors.foreground,
                      },
                    ]}
                  >
                ...
                <View style={[styles.prioRow, { flexWrap: "wrap" }]}>
                  <TouchableOpacity
                    style={[
                      styles.prioChip,
                      {
                        backgroundColor: !delegateToEmail
                          ? colors.primary
                          : colors.muted,
                      },
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.prioChipText,
                        {
                          color: !delegateToEmail ? "#fff" : colors.foreground,
                        },
                      ]}
                    >
                      Unassigned
              ...
              {loading ? (
                ...
                <>
                  ...
                  <Text style={styles.bigBtnText}>
                    {isOwner ? "Delegate Task" : "Send Task"}
                  </Text>
                </>
              )}
            </TouchableOpacity>
